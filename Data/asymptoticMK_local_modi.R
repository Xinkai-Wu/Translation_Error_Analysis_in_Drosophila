...
# d, d0, input tab file, x intervals should be provided.
args <- commandArgs(T)
d <- as.numeric(args[1])
d0 <- as.numeric(args[2])
df <- read.csv(args[3],sep="\t")
f <- df$x
p <- df$pN
p0 <- df$pS

asymptoticMK.Drosophila <- function(xlow=0.05, xhigh=0.95, output="table", ...)
{
...
setwd("/lustre/user/lulab/doushq/wuxk/cub_assay/flyProteomes/Scripts/MK_test")		
# set the directory where you want the PDF files to appear
